...
export const POST = async (request: NextRequest) => {
  ...
  const foundcourseNo = await prisma.course.findFirst({ //or findUnique
  ...
  //add course to database (enrollments collection)
    await prisma.enrollment.create({
      data: {courseNo: courseNo,studentId: studentId, },
    })
...
export const DELETE = async (request: NextRequest) => {
  ...
  await prisma.enrollment.delete({
    where: {
    courseNo_studentId: { courseNo: courseNo,studentId: studentId, },
    },
  });
